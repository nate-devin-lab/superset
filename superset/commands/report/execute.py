...
class BaseReportState:
    ...
    def __init__(
        ...
        self._start_dttm = datetime_utc_now()
    ...
    def update_report_schedule(self, state: ReportState) -> None:
        ...
        self._report_schedule.last_eval_dttm = datetime_utc_now()
        ...
        try:
            ...
            for screenshot in screenshots:
                if imge := screenshot.get_screenshot(user=user):
                    imges.append(imge)
            elapsed_seconds = (datetime_utc_now() - start_time).total_seconds()
            logger.info(
...
class ReportWorkingState(BaseReportState):
    ...
    def next(self) -> None:
        if self.is_on_working_timeout():
            ...
            elapsed_seconds = (
                (datetime_utc_now() - last_working.end_dttm).total_seconds()
                if last_working
                else None
            )
            logger.error(
